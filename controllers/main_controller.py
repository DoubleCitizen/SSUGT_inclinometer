...
class Ui_MainWindowController(QMainWindow, main.Ui_MainWindow, QObject):
    """Drives real-time inclinations bounding module UI overlays mapping API bindings."""
    ...
    def send_frame_in_graphics_view(self, frame: np.ndarray):
        """Injects computed numpy matrices transforming overlays translating output."""
    ...
    def start_stream(self, cap):
        """Binds incoming local tracking structures initializing visual overlays mapping."""
    ...
    def stop_stream(self):
        """Purges execution hooks stopping visual capture streams flushing buffers."""
    ...
    @staticmethod
    def open_directory():
        """Spawns OS native explorer interacting cached folders configurations."""
        path = os.getcwd() + '\\data'
